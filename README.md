...
会话列表按真实接口返回排序。点开会话会先清 Web 本地未读角标，再通过本地 Node SignalR 桥调用官方同源逻辑 `ConsumeMessage(contactId, 0)`；只有桥接失败时才依次兜底浏览器 SignalR 和 HTTP `/ChatContent/ConsumeMessage`。右键“清空列表”后会调用真实清空接口，并短时间过滤刚清空的会话，避免自动刷新马上把列表弹回来。历史列表只展示 `/Contact/GetContactList(isHistory=true)` 的接口数据，不再混入 Web 本地缓存。
